...
- `Documentation` now holds a generation `DateTimeImmutable` (defaults to now, injectable via constructor) exposed through `getDate()`

## [2.2.2] - 2026-06-02

### Fixed

- Fix `TypeError` in `PathTreatment` when a relative link climbs above the documentation root: `b_resolve_absolute_path()` returns `null` and the original path is now kept unchanged instead of crashing
